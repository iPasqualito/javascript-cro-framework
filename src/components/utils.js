const ra_utils = function(logger) {
	...
	return {
		awaitNode: (parameters, callback) => {
			logger.log("awaitNode: start", parameters);
			try {
				...
				logger.log("awaitNode: ready");
			}
		},
		isMobile: function () {
			logger.info("isMobile: start");
			...
			logger.info("isMobile: ready", mobile);
			return mobile;
		},
		addNode: function (type, attrs) {
			logger.info("addNode: start");
			const el = document.createElement(type);
			this.setAttributes(el, attrs);
			logger.info("addNode: ready", el);
			return el;
		},
		addStyle: function (css, id) {
			logger.info("addStyle: start");
			try {
				...
				logger.info("addStyle: ready");
			}
		},
		setAttributes: function (e, a) {
			logger.info("setAttributes: start");
			Object.entries(a).map(([key, value]) => {
				logger.log("setAttributes", key, value);
				e.setAttribute(key, value);
			});
			logger.info("setAttributes: ready");
		},
		editQueryParam: function (parameters) {
			...
			logger.info("editQueryParam ready", url.toString());
			return url.toString();
		}
	}
