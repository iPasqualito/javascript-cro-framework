const ra_utils = (logger) => {
	...
	return {
		...
		getScreenSize: (screenWidth = window.innerWidth) => logger.info("utils: getScreenSize", screenWidth) || (screenWidth < 480 ? "small" : (screenWidth >= 480 && screenWidth <= 1024) ? "medium" : "large"),
		isTouchEnabled: () => logger.info("utils: isTouchEnabled") || ('ontouchstart' in window) || (navigator.maxTouchPoints > 0) || (navigator.msMaxTouchPoints > 0),
		isMobile: () => {
